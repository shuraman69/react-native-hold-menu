...
const HoldItemComponent = ({
  ...
  const menuHeight = (0, _react.useMemo)(() => {
    ...
    return (0, _calculations.calculateMenuHeight)(items.length, itemsWithSeparator.length, maxVisibleItems);
  ...
  const calculateTransformValue = () => {
    ...
    const hasCustomView = !!renderCustomView; // For tall items (>70% of screen height), position so item bottom is at screen center
    // Menu will appear below the center point

    const TALL_ITEM_THRESHOLD = 0.7;
    const isTallItem = itemRectHeight.value > screenH * TALL_ITEM_THRESHOLD; // If custom view exists, center item on screen
